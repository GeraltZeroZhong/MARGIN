#!/usr/bin/env python
"""Export frozen sequence representations for the cross-platform panel."""
...
def main() -> None:
    ...
    for model_id in requested:
        command = [
            ...
            "--config",
            str(stability.paths.generalization_config),
            "--model-id",
            model_id,
            "--queries",
            str(queries),
        ...
        if model_id == "esm1b_650M":
            command.extend(
                ["--loader", "hf_esm", "--checkpoint", str(stability.paths.esm1b_hf_checkpoint)]
            )
