#!/usr/bin/env python
"""Export frozen CARP features and ESM2 sequence scores for counterfactual study."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        choices=["all", "carp_640M", "esm2_150M"],
        default="all",
    )
    ...
    if lock.get("status") != "FROZEN_BEFORE_LOCKED_PANEL_MODEL_SCORING":
        raise RuntimeError("counterfactual study protocol lock is missing")
