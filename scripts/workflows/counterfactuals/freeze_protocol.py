#!/usr/bin/env python
"""Freeze the counterfactual workflow before any locked-panel model scoring."""
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(f"cannot freeze after locked-panel model scoring: {existing}")
    required = [
    ...
    tables = []
    for name in ("domains", "variants", "residues", "query_rows", "exclusions"):
    ...
    write_json(
        lock_path,
        {
            **runtime_manifest(config.paths.project_root),
            ...
            "status": "FROZEN_BEFORE_LOCKED_PANEL_MODEL_SCORING",
            "frozen_files": copies,
            "locked_tables": tables,
            "forbidden_confirmation_panel": "generalization_opened_52_assays",
        },
    )
