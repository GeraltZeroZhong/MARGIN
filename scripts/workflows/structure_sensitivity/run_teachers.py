#!/usr/bin/env python
"""Score every frozen structure-sensitivity study structure role with the three paired teachers."""
...
def main() -> None:
    ...
    structures = pd.read_parquet(config.paths.run_dir / "teacher_requests/structures.parquet")
    scores = scores.merge(
        structures[["structure_id", "analysis_population"]],
        on="structure_id",
        validate="many_to_one",
    ).sort_values(["teacher_id", "structure_role", "domain_id", "position"], ignore_index=True)
    write_parquet(output / "scores.parquet", scores)
    coverage = (
        scores.groupby(["teacher_id", "structure_role"], observed=True)
    ...
    write_json(
        output / "manifest.json",
        {
            **runtime_manifest(config.paths.project_root),
            ...
            "status": "STRUCTURE_SENSITIVITY_ALTERNATE_STRUCTURE_SCORES_COMPLETE",
            "outcomes_read": False,
            "teacher_inference_seed": cross.seed,
            "structure_sensitivity_perturbation_and_bootstrap_seed": config.seed,
